...
for flight in flights:
    ...
    # Draw flight path
    folium.PolyLine(
        locations=[(flight["src_lat"], flight["src_lon"]), (flight["dst_lat"], flight["dst_lon"])],
        color="black",
        weight=2.5,
        opacity=1
    ).add_to(m)
...
for flight in flights:
    ...
    ax.scatter(flight["src_lon"], flight["src_lat"], color="blue", label=f"Source: {flight['source']} ({flight['src_country']})")
    ...
    ax.text(flight["src_lon"], flight["src_lat"], f"{flight['source']} ({flight['src_country']})", fontsize=12, verticalalignment="bottom", color="blue")
    ...
    # Draw flight path
    ax.plot([flight["src_lon"], flight["dst_lon"]], [flight["src_lat"], flight["dst_lat"]], linestyle="--", color="black")
